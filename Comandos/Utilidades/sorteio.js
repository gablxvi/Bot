...
Clique no botão para parcipar.\n**Boa sorte!!!**`)
      ...
      coletor.on("collect", (i) => {
        ...
        if (i.customId === "botao") {
          ...
          if (click.includes(i.user.id)) return i.reply({ content: `Olá você ja está participando do sorteio.`, ephemeral: true });

          click.push(i.user.id);

          interaction.editReply({ embeds: [embed] });

          i.reply({ content: `Olá você entrou no sorteio.`, ephemeral: true });
        }
